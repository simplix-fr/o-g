<?php
	...
	if($adaptive == "yes")
	{
		$acc = get_permalink(get_option('ProjectTheme_my_account_personal_info_id'));
		$paypal_email = get_user_meta($current_user->ID,'paypal_email',true);
		
		if(empty($paypal_email))
		{
			echo '<div class="padd10"><div class="padd10">';
			echo sprintf(__('You need to set your paypal email before making a proposal. <br/><a href="%s">Go to your account area</a>.','ProjectTheme'), $acc);
			echo '</div></div>';
			exit;
		}
	}
